...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  // Sin BLOB_READ_WRITE_TOKEN no hay forma de subir nada — en vez de que
  // cada tenant falle uno por uno con el mismo error, se corta acá con un
  // mensaje claro. En producción real (deploy en Vercel) este token se
  // inyecta solo; en dev local no está configurado, así que ahí este cron
  // no puede correr — el mensaje deja claro por qué.
  if (!process.env.BLOB_READ_WRITE_TOKEN) {
    const message = "BLOB_READ_WRITE_TOKEN no está configurado: no se puede subir ningún backup.";
