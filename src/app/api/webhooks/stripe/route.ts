...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  // Todo el procesamiento post-verificación de firma va envuelto en un
  // try/catch propio: antes de esto, una excepción acá (ej. la base caída a
  // mitad de un $transaction) se propagaba sin dejar rastro más que el log
  // efímero de Vercel. Estos webhooks mueven plata real, así que además del
  // 500 -> reintento automático de Stripe, queda registrado en ErrorLog.
  try {
    ...
    await logError(err, { eventType: event.type });
